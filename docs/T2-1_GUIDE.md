...
```
GitHub Enterprise  ─┐
Confluence          ├─→ collector → preprocessor → chunker → embedder → indexer → Febrix VectorDB
두드림 (CR 이력)    ─┘
```
...
| 소스 | 데이터 유형 | 청킹 전략 | 컬렉션 |
|------|------------|-----------|--------|
| GitHub Enterprise | Python · SQL · 설정 파일 | AST 함수/클래스 단위 | `dev_agent_github_code` |
| Confluence | 운영 문서 · 설계서 · 매뉴얼 | 헤딩(##) 기준 섹션 | `dev_agent_confluence_docs` |
| 두드림 | 과거 CR 처리 이력 | 1 CR = 1 청크 | `dev_agent_cr_history` |
...
*작성일: 2026-06-24 | T2-1 데이터 수집 및 전처리 | 다음: T2-2 하이브리드 검색 (BM25 + Dense)*
